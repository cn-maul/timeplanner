...
一个本地运行的个人时间规划工具：录入固定重复的**周期事件**（例会、课程等），系统自动生成每周时间表并识别所有**剩余空闲时段**；在空闲时段里自由安排工作、学习、休闲，形成完整的**日计划**与**周计划**。
...
- **分类与统计**：安排分工作 / 学习 / 休闲三类配色；周、日两个维度的固定/已计划/空闲时长与分类占比。
...
安排分类：工作（靛蓝）、学习（青）、休闲（粉）。名称可留空，保存后自动显示分类名。
